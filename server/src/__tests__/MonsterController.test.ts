...
function makeRoom(overrides: Partial<GameRoomType> = {}): GameRoomType {
  return {
    id: 'room-1',
    name: 'Test Room',
    players: [],
...
function makePlayers(players: Partial<Player>[]): Map<string, Player> {
...
describe('MonsterController — initialization', () => {
  test('initializes with room monster state', () => {
    ...
    const controller = new MonsterController(room, players);
  ...
  test('monster starts in IDLE state', () => {
    ...
    const players = makePlayers([{}]);
    const controller = new MonsterController(room, players);
    expect(controller.getMonster().state).toBe(MonsterState.IDLE);
  });
...
describe('MonsterController — IDLE state', () => {
  test('does nothing when no alive players exist', () => {
    ...
    expect(room.monster.state).toBe(MonsterState.IDLE);
  });
  ...
  test('transitions to STALKING when a lone player is within detection radius', () => {
    ...
    controller.update(0.05);
    expect(room.monster.state).toBe(MonsterState.STALKING);
  });
...
describe('MonsterController — speed based on being watched', () => {
  test('monster is slower when not watched', () => {
    const room = makeRoom();
    room.monster.state = MonsterState.HUNTING;
...
describe('MonsterController — ATTACKING state', () => {
  test('isAttacking() returns targetPlayerId when in ATTACKING state', () => {
    const room = makeRoom();
    room.monster.state = MonsterState.ATTACKING;
    room.monster.targetPlayerId = 'player-1';
  ...
  test('monster transitions to ATTACKING when close enough to target', () => {
    ...
    controller.update(0.05);
    expect(room.monster.state).toBe(MonsterState.ATTACKING);
  });
});
...
describe('MonsterController — teleport', () => {
  test('monster does not teleport when being watched', () => {
    const room = makeRoom();
    room.monster.position = { x: 0, y: 0, z: 0 };
    room.monster.state = MonsterState.STALKING;
    ...
    (controller as any).lastTeleportTime = 0;
    (controller as any).watchedByPlayers = new Set(['player-1']);

    const target = makePlayers([{ position: { x: 30, y: 0, z: 0 } }]).get('player-1')!;
    (controller as any).tryTeleport(target);
  ...
  test('monster teleports when not watched and far enough from target', () => {
    ...
    const controller = new MonsterController(room, players);
    (controller as any).watchedByPlayers = new Set(); // nobody watching
    (controller as any).lastTeleportTime = 0; // cooldown expired

    const target = players.get('player-1')!;
    (controller as any).performTeleport(target);

    // Monster should have moved closer to player
    const newDist = Math.sqrt(
      (room.monster.position.x - target.position.x) ** 2 +
  ...
  test('teleport respects cooldown', () => {
    ...
    const players = makePlayers([{ position: { x: 30, y: 0, z: 0 } }]);
    const controller = new MonsterController(room, players);
    (controller as any).watchedByPlayers = new Set();
    (controller as any).lastTeleportTime = Date.now(); // just teleported
...
describe('MonsterController — STALKING state', () => {
  ...
  test('transitions to HUNTING based on pages ratio', () => {
    ...
    (controller as any).stalkPhaseStart = Date.now() - 25000;

    controller.update(0.05);
    expect(room.monster.state).toBe(MonsterState.HUNTING);
  });
});
...
describe('MonsterController — distance helper', () => {
  test('calculates correct distance to player', () => {
    ...
    const dist = (controller as any).getDistanceToPlayer(players.get('player-1'));
