...
function mockIo(): jest.Mocked<Server> {
  const emittedEvents: Array<{ event: string; data: any }> = [];
  const toMap = new Map<string, any>();

  const toFn = (roomId: string) => ({
...
function createRoomWithPlayers(count: number): { room: GameRoom; io: any } {
  const io = mockIo();
...
describe('GameRoom — initialization', () => {
  test('creates room with correct initial state', () => {
    const io = mockIo();
    const room = new GameRoom(io, 'Horror Room');
  ...
  test('monster starts in IDLE state at initial position', () => {
    ...
    expect(room.state.monster.state).toBe(MonsterState.IDLE);
    expect(room.state.monster.position).toEqual({ x: -50, y: 0, z: -50 });
  });

  test('each room gets a unique ID', () => {
    const io = mockIo();
    const room1 = new GameRoom(io, 'Room 1');
    const room2 = new GameRoom(io, 'Room 2');
...
describe('GameRoom — startGame', () => {
  ...
  test('does not start without players', () => {
    const io = mockIo();
    const room = new GameRoom(io, 'Empty');
    room.startGame();
    expect(room.state.phase).toBe(GamePhase.LOBBY);
  });
...
describe('GameRoom — win/lose conditions', () => {
  ...
  test('stop() clears update intervals', () => {
    ...
    expect((room as any).updateInterval).toBeNull();
    expect((room as any).ambientInterval).toBeNull();
  });
});
